...
exports.getAllTours = factory.getAll(Tour)
    // exports.getAllTours = catchAsync(async(req, res, next) => { //http://127.0.0.1:4301/api/v1/tours
...
//     // hier braucht es nicht, weil es kein fehler gibt, den anfrage richtig, db auch richtig, sonst mongodb mach eigenen fehler
...
//     //video 153, so könnte man implementieren, aber dublicated code, desshalb im Tour-Model eine query-Middleware
//     // .populate({
//     //     path: 'guides',
...
//     // const tour = await Tour.findById(req.params.id).populate({//ausgelagert weil dublicatet code in query-Middleware in TourModel
//     //         path: 'guides',
//     //         select: '-__v -passwordChangeAt' // was man nicht sehen möchte bei output
//     //     })
//     //const tour = await Tour.findById(req.params.id).populate('guides') //video 153 damit nicht nur die referenzID von User in Tour, auch seine Daten
...
// /tours-distance/233/latlng/-40,45/unit/mi
exports.getToursWithin = catchAsync(async(req, res, next) => {
    ...
    if (!lat || !lng) {
        next(new AppError('Please provide latitut and longitude in the format lat, lng.', 400)) //400 = bad request
    }
...
exports.getDistances = catchAsync(async(req, res, next) => {
    ...
    const distances = await Tour.aggregate([{
        $geoNear: { // muss erstes sein gemäss dok//fist stage in pipeline// geheime sachen in tourmodel middleware auskommentiert
            near: {
                type: 'Point',
